...
class BannerDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->addColumn('image', function ($banner) {
                return $banner->image_path
                    ? '<img src="'.asset('storage/'.$banner->image_path).'" class="w-24 h-12 object-cover rounded-lg shadow-sm border border-slate-100 mx-auto">'
            ...
            ->addColumn('status', function ($banner) {
                return $banner->status
                    ? '<span class="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-1 text-xs font-satoshi-semibold text-emerald-700 ring-1 ring-inset ring-emerald-600/10">Active</span>'
                    : '<span class="inline-flex items-center gap-1 rounded-full bg-rose-50 px-2 py-1 text-xs font-satoshi-semibold text-rose-700 ring-1 ring-inset ring-rose-600/10">Inactive</span>';
            })
            ->addColumn('action', function ($row) {
                ...
                if (auth()->user()->can('Banner Delete')) {
                    $delete = '
                        <form action="'.route('banner.destroy', $row->uuid).'"
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('title')->title('Title')->addClass('px-4 py-3 border-b border-slate-200 text-slate-900 font-semibold'),
            Column::make('subtitle')->title('Subtitle')->addClass('px-4 py-3 border-b border-slate-200 text-slate-500'),
            Column::make('link_url')->title('Link URL')->addClass('px-4 py-3 border-b border-slate-200 text-slate-500'),
            Column::make('status')->title('Status')->addClass('px-4 py-3 border-b border-slate-200 text-center'),
            Column::make('sort')->title('Sort Order')->addClass('px-4 py-3 border-b border-slate-200 text-center'),
