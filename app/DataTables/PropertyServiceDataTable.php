...
class PropertyServiceDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('icon_display', function ($row) {
                if ($row->icon) {
                    return '<div class="w-9 h-9 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center mx-auto text-xl shadow-xs"><i class="' . e($row->icon) . '"></i></div>';
                ...
                return '<div class="w-9 h-9 rounded-lg bg-slate-100 text-slate-400 flex items-center justify-center mx-auto text-xl"><i class="ri-service-line"></i></div>';
            })
            ...
            ->addColumn('action', function ($row) {
                ...
                if (auth()->user()->can('Property Service Update')) {
                    $edit = '<a href="' . route('property_services.edit', $row->uuid) . '"
                ...
                if (auth()->user()->can('Property Service Delete')) {
                    $delete = '
                        <form action="' . route('property_services.destroy', $row->uuid) . '"
            ...
            ->rawColumns(['icon_display', 'price_formatted', 'category_badge', 'status', 'action']);
    }

    /**
     * Get query source of dataTable.
     */
    public function query(PropertyServices $model)
    {
    ...
    {
        return $this->builder()
            ->setTableId('property-services-table')
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('name')->title('Service Name')->addClass('px-4 py-3 border-b border-slate-200 text-slate-900 font-semibold'),
            Column::make('category_badge')->title('Category')->addClass('px-4 py-3 border-b border-slate-200 text-center'),
            Column::make('price_formatted')->title('Price')->addClass('px-4 py-3 border-b border-slate-200'),
    ...
    {
        return 'PropertyServices_' . date('YmdHis');
    }
}
