...
class DestinationDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('image_display', function ($row) {
                if ($row->image_path) {
                    $src = \Illuminate\Support\Str::startsWith($row->image_path, ['http://', 'https://'])
                        ? $row->image_path
                        : asset('storage/' . $row->image_path);
                    return '<img src="' . $src . '" class="w-14 h-10 object-cover rounded-lg shadow-xs mx-auto border border-slate-200">';
                }
                return '<div class="w-14 h-10 rounded-lg bg-slate-100 text-slate-400 flex items-center justify-center mx-auto text-lg"><i class="ri-map-pin-2-line"></i></div>';
            })
            ->addColumn('tags_badge', function ($row) {
                if (!$row->tags) return '<span class="text-slate-400 text-xs">-</span>';
                $tags = explode(',', $row->tags);
                $html = '<div class="flex flex-wrap gap-1 justify-center">';
                foreach ($tags as $t) {
                    $html .= '<span class="inline-flex items-center rounded-md bg-slate-100 px-2 py-0.5 text-[11px] font-satoshi-medium text-slate-700">' . e(trim($t)) . '</span>';
                }
                $html .= '</div>';
                return $html;
            })
            ...
            ->addColumn('action', function ($row) {
                ...
                if (auth()->user()->can('Destination Update') || auth()->user()->hasRole('Super Admin')) {
                    $edit = '<a href="' . route('destination.edit', $row->uuid) . '"
                ...
                if (auth()->user()->can('Destination Delete') || auth()->user()->hasRole('Super Admin')) {
                    $delete = '
                        <form action="' . route('destination.destroy', $row->uuid) . '"
            ...
            ->rawColumns(['image_display', 'tags_badge', 'status', 'action']);
    }
    ...
    {
        return $model->newQuery()->orderBy('sort', 'asc');
    }
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('image_display')->title('Photo')->orderable(false)->searchable(false)->width(80)->addClass('text-center px-4 py-3 border-b border-slate-200'),
            Column::make('name')->title('Destination Name')->addClass('px-4 py-3 border-b border-slate-200 text-slate-900 font-semibold'),
            Column::make('tags_badge')->title('Tags / Features')->addClass('px-4 py-3 border-b border-slate-200 text-center'),
            Column::make('attraction')->title('Main Attraction')->addClass('px-4 py-3 border-b border-slate-200 text-slate-500'),
